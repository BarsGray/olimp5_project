...
add_action('admin_enqueue_scripts', function ($hook) {
    ...
    $inline_css = "#post-type-priority .pt-item {cursor: move;cursor: -webkit-grabbing;cursor: grabbing;user-select: none;}.ui-sortable-placeholder {border: 2px dashed #ccd0d4 !important;background: #f6f7f7 !important;visibility: visible !important;height: 40px;}";
    wp_add_inline_style('common', $inline_css);
});
add_action('wp_print_scripts', function(){
    global $wp_scripts;
    if (is_user_logged_in()) {echo "<script>console.log('ENQUEUED:', ".json_encode(wp_list_pluck($wp_scripts->queue, null)).");</script>";}
});
...
function rb_register_styles() {
	...
	wp_enqueue_style( 'top-main-style',  get_stylesheet_directory_uri() . '/assets/css/top-main-style.css', array(), '1.1.8' );
	...
	wp_enqueue_script( 'script', get_stylesheet_directory_uri() . '/assets/js/addon.js', array( 'sticky', 'swiper', 'inputmask', 'popup', 'fancybox' ), '1.0.8', true );
